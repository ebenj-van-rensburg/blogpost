...
function App() {
  ...
  const handleSubmit = (event) => { // Prevent page reload
    event.preventDefault(); // Extract username and password from the form
    var { uname, pass } = document.forms[0]; // Find user login info from the database
    const userData = database.find((user) => user.username === uname.value); // Compare user info
    if (userData) {
      if (userData.password !== pass.value) { // If password is invalid, set error message
        setErrorMessages({ name: "pass", message: errors.pass });
      } else { // If username and password are correct, set isSubmitted to true
        setIsSubmitted(true);
      }
    } else { // If username is not found in the database, set error message
